...
export class ResumesService {
  ...
  async createPdf(id: string) {
    ...
    Object.keys(data.Educations).forEach((key) => {
      switch (
        data.Educations[key].schooling &&
        data.Educations[key].situation
      ) {
        case 1:
          return (
            (data.Educations[key].schooling = 'Ensino Fundaental'),
            (data.Educations[key].situation = 'Parada')
          );
        case 2:
          return (
            (data.Educations[key].schooling = 'Ensino Médio'),
            (data.Educations[key].situation = 'Completa')
          );
        case 3:
          return (
            (data.Educations[key].schooling = 'Ensino Superior'),
            (data.Educations[key].situation = 'Em Progresso')
          );
      }
    });
    Object.keys(data.Languages).forEach((key) => {
      ...
      switch (data.Languages[key].degreeOfInfluence) {
        case 1:
          return (data.Languages[key].degreeOfInfluence =
    ...
    Object.keys(data.Skills).forEach((key) => {
      data.Skills[key];
      switch (data.Skills[key].typeOfPeriod && data.Skills[key].seniority) {
        case 1:
          return (
            (data.Skills[key].typeOfPeriod = 'Mês(s)'),
            (data.Skills[key].seniority = 'Júnior')
          );
        case 2:
          return (
            (data.Skills[key].typeOfPeriod = 'Ano(s)'),
            (data.Skills[key].seniority = 'Full Stack')
          );
        case 3:
          return (data.Skills[key].seniority = 'Sênior');
      }
      if (data.Skills[key].currentPosition === 1) {
        data.Skills[key].currentPosition = 'Posição Atual';
      } else {
        data.Skills[key].currentPosition = 'Posição Antiga';
      }
    });
    ...
    data.Educations.forEach(function (education) {
      ...
      educationList.push({
        text: `Instituição: ${education.institution}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
      educationList.push({
        text: `Curso: ${education.course}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
      educationList.push({
        text: `Situação:  ${education.situation}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
    });
    ...
    data.Languages.forEach(function (language) {
      languageList.push({
        text: `Nome: ${language.languageName}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
      languageList.push({
        text: `Grau: ${language.degreeOfInfluence}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
    });
    ...
    data.Skills.forEach(function (skill) {
      skillList.push({
        text: `Tecnologia: ${skill.technology}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
      skillList.push({
        text: `Tempo de Experiência: ${skill.yearsExperience}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
      skillList.push({
        text: `Período: ${skill.typeOfPeriod}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
      skillList.push({
        text: `senioridade:  ${skill.seniority}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
      skillList.push({
        text: `Posição:  ${skill.currentPosition}`,
        margin: [0, 6, 4, 0],
        fontSize: 12,
      });
    });
          ...
          columns: [
            {
              stack: [
                ...
                {
                  text: 'Data de Aniversário: ' + data.birthDate,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
                  text: 'Gênero: ' + obj.gender,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
                  text: 'Estado civil: ' + obj.maritalStatus,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                ...
                },
                {
                  text:
                    'Telefone: ' +
                    `(${data.Phone.ddd}) +${data.Phone.ddi} ${data.Phone.phoneNumber}`,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
                ...
                {
                  text: 'Estado: ' + data.Address.state,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
                  text: 'Cidade: ' + data.Address.city,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
                  text: 'Bairro: ' + data.Address.district,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
                  text: 'Rua: ' + data.Address.street,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
                  text: 'Número: ' + data.Address.number,
                  margin: [0, 6, 4, 0],
                  fontSize: 10,
                },
                {
